...
const TOML_PATH: PathBuf = PathBuf::from("../../toml");
const OUT_PATH: PathBuf = PathBuf::from("../../std/src");
const ROOT: &str = r#"# Mufi Lang v{version} [{release} Release]

## List of Modules
"#;

const MOD_TEMPLATE: &str = r#"# **Module Name**

## List of Functions
"#;

const TEMPLATE: &str = r#"## <a id="{function}">*{function}*</a>
> {description}
...
impl RootModule{
    ...
    fn combine(&self) -> String{
        ...
        for i in &self.modules{
            list.push(format!("[{}](./{}.md)", &i.name, &i.name))
        }
